import { getMimeType, LiveReloadServer, WebServer } from "h2server";
import { color } from "tutils/ansi";
...
export class DevServer extends WebServer {
    ...
    async start(ignoreError) {
        try {
            await super.start();
            this.builder.logger.info(`${color(`服务已启动: `, 96 /* brightCyan */)} ${this.url}`, true);
        }
        catch (e) {
            if (!ignoreError) {
    ...
    async close() {
        ...
        await super.close();
        this.builder.logger.info(color(`服务已停止`, 96 /* brightCyan */), true);
    }
    /**
     * 默认路由
            ...
            switch (asset.type) {
                case 1 /* file */:
                    ...
                    response.writeHead(200, {
                        "Content-Type": (_b = getMimeType(url, { ".ts": "text/typescript", ...this.mimeTypes })) !== null && _b !== void 0 ? _b : "text/html",
                        "ETag": asset.hash,
                        ...this.headers
                    });
